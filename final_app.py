...
if (SHOW_CAR && ZONES.length > 0) {{
  ...
  // ── Nearest-neighbour sort: visit every zone in geographic order ──
  function nnSort(start, zones) {{
    const rem = [...zones], out = [];
    let cur = start;
    while (rem.length) {{
      let bi = 0, bd = Infinity;
      rem.forEach((z,i) => {{ const d=haversineM(cur[0],cur[1],z.lat,z.lng); if(d<bd){{bd=d;bi=i;}} }});
  ...
  // ── Densify: insert N smooth points between A and B ──
  function densify(a, b, n) {{
    const pts=[];
    for(let i=1;i<=n;i++){{const t=i/(n+1); pts.push([a[0]+(b[0]-a[0])*t, a[1]+(b[1]-a[1])*t]);}}
    return pts;
  }}

  // ── Build ONE continuous route: start from DB path[0] if available ──
  const startPt = (PATHS.length && PATHS[0].coords && PATHS[0].coords.length)
    ? PATHS[0].coords[0] : [ZONES[0].lat, ZONES[0].lng];

  const ordered = nnSort(startPt, [...ZONES]);
  const INTERP  = 60; // smooth points between each zone
  ...
  const TARGET_MS = 45000;
  const BASE_DELAY = Math.max(30, Math.floor(TARGET_MS / totalPts));
  ...
  function step() {{
    if (idx >= allCoords.length) {{
      ...
      addAlert('🏁 All zones visited!', `🏁 <b>Simulation complete — ${{ordered.length}} zones visited!</b>`, 'safe');
      return;
    }}
  ...
  addAlert('🟢 Simulation started',
    `🟢 <b>Simulation started — visiting ${{ordered.length}} zones automatically</b>`, 'safe');
  setTimeout(step, 500);
}}
</script>
</body>
...
def main():
    ...
    with st.sidebar:
        st.markdown("## 🚦 Road Risk Navigator")
